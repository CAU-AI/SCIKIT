...
cov_data = np.cov(data)      #583 * 583

eigenvalue = np.linalg.eigvals(cov_data)     #583

a=eigenvalue.tolist()
b=eigenvalue.tolist()

'''
a.sort(reverse=True)                    #sort에 오류발생
...
for i in range(0, len(cov_data)):
    first_eigenvector = np.c_[first_eigenvector, [cov_data[i][first_eigenvector_index]]]       #1*583
...
first_list = first_eigenvector.tolist()
...
new_data2 = np.dot(data,second_eigenvector)
