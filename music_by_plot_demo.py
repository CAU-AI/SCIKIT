...
scaler = preprocessing.Imputer(missing_values='NaN', strategy='mean', axis=0).fit(sub_data.astype(float))
sub_data = scaler.transform(sub_data.astype(float))
...
for i in demo_pred:
    if i == '리드미컬':
        ...
        train_model = gnb.fit(sub_data, sub_answer)       #61, 71(2), 76(2), 85(3), 90, 95
