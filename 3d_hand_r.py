...
train_x, test_x, train_y, test_y, train_z, test_z, train_a, test_a = train_test_split(X, Y, Z, A, test_size=0.1)
...
for i in range(0, i_length):
    loc_min = 9999999999999
    loc_min_answer = [0]
    for j in range(0, len_train - 1):
        dist_x, path_x = fastdtw(test_x[i], train_x[j], dist=euclidean)
        dist_z, path_z = fastdtw(test_z[i], train_z[j], dist=euclidean)
    ...
    print(str(i) + " : " + np.array(test_a[i])[0])
    f.write(str(np.array(test_a[i])[0])+ '\n')
    if loc_min_answer == np.array(test_a[i])[0]:
        print("--correct")
        correct = correct + 1
        apb[ord(np.array(test_a[i])[0])-97].append(True)
    else:
        apb[ord(np.array(test_a[i])[0])-97].append(False)
print("time : " + str(time.time() - start_time))
print("Accuracy : " + str(correct/i_length))
...
for i in apb:
    ...
    for j in i:
        ...
        if j:
            ans += 1
    try:
        print(chr(alp) + ":" + str(ans/cnt) + " (" + (str(ans) + "/" + str(cnt) + ")"))
    except ZeroDivisionError as e:
        print(chr(alp) + ":" + "no case (0)")
